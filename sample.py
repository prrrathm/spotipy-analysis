...
def main():
    ...
    for query in queries:
        ...
        while idx < num_tracks_per_query:
            ...
            API_search_request(query, 'track', 50, idx, ltrack,
                               song_ids, artist_ids, album_ids)
            n += 1
...
def API_search_request(keywords, search_type, results_limit, results_offset, ltrack, song_ids, artist_ids, album_ids):
    ...
    url = 'https://api.spotify.com/v1/search?q=year:' + keywords + \
        '&type=' + search_type + '&offset=' + off + '&limit=' + lim
...
def API_get_audio_feature(songids, audioF):
    ...
    r = requests.get(
        url, headers={"Accept": "application/json", "Authorization": access_token})
